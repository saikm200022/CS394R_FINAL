...
while episode < num_episodes:
    while not p.game_over():
        ...
        action_prime, idx_prime = choose_action(v_idx, h_idx, vel_idx, Q)

        next_state_Q = 0
        if not p.game_over():
            next_state_Q = Q[next_state_vertical_dist - min_vert, next_horizontal_dist, next_velocity - min_vel, idx_prime]

        Q[v_idx, h_idx, vel_idx, idx] += alpha * (reward + gamma * next_state_Q - Q[v_idx, h_idx, vel_idx, idx])
...
# change file name to reflect which epsilon was used
csv_file = 'csvs/sarsa_eps_0p0_learning.csv'
pkl_file = 'pkls/sarsa_eps_0p0.pkl'
